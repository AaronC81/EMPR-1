...
{
	const uint32_t LED_array[4] = {LED_0, LED_1, LED_2, LED_3};
	static uint32_t count = -1;
	static uint32_t timer = 0;
	if (timer++ >= 10)
	{
		GPIO_ClearValue(1, LED_array[count % 4]);
		count++;
		GPIO_SetValue(1, LED_array[count % 4]);
		timer = 0;
	}
}
...
{
	serial_init();
	write_usb_serial_blocking("Stage 3 test code\n\r",19);
